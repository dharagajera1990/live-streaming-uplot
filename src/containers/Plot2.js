...
export default function Plot2({width,height}) {
  ...
  let strokeColor = "blue";  let range = [2, 6];
  if(localStorage.getItem('chart2Data') !== null){
     strokeColor ="yellow";
     range= [1, 5];
  }
  ...
  const handleDelete = ()=>{
    ...
    if(boxResult === true){
      localStorage.removeItem("chart2Data");
      setPlot(plot);
    }
  }
  ...
  return (
    <div>
      ...
      {toggle && <Sidebar close={() => setToggle(false)} chart="chart2" />}
      <div ref={plotRef} />
      <Data chart="chart2" />
    </div>
  );
